...
文件头包含 `title`、`date`、`tags`、`sources` 和 `status`。最终文章只保留成文，不混入 Capability Manifest、Context Envelope、Reader Contract、Source Brief、Source Catalog、Dialogue Matrix、Candidate Frame Brief、Comprehension Gate 探针、Impact Brief、Article Closure Contract 或内部评分表。交付报告会说明选中框、胜出理由、有实质差异的备选框、预留测试结果、Comprehension Gate 状态、检测到的宿主、实际使用的背景来源类别，以及 Impact Pass 的结果；deep-read 还会记录最终文件的 Article Integrity 结果。
...
`evals/evals.json` 包含 12 个回归场景，除原有的素材、项目、领域和 Impact 路径外，还覆盖错误初始问题的修复，以及闭卷重建、新例预测和反例收缩。`evals/frame-quality.md` 定义候选框准入、预留测试、理解检验与成文追溯的评估方法。
...
`ImpactMode` 可取 `personal`、`question` 或 `none`。它会机械检查文章 frontmatter、字面意义层标题、pre-reveal 文件及其隐私边界、交付报告中的 Comprehension Gate 状态和 hold-out 时间顺序状态；对于 deep-read，还会对实际写入的 Markdown 执行 Article Integrity 检查。非零退出码表示本次运行不能声明通过。
