...
它不会直接把素材压成摘要。它会先建立证据模型，再从素材里找出几种真正成立的看法，测试它们的边界，最后选一副框来组织全文。
...
Survey 不预设每个领域都有研究纲领或范式革命。它会在方法族、瓶颈与价值流、争议轴、研究纲领、时间演化之间做选择，让主体结构跟着证据走。
...
克隆仓库到宿主的 skills 目录，确保 `SKILL.md` 位于 weave 目录根部。
...
文件头包含 `title`、`date`、`tags`、`sources` 和 `status`。最终文章只保留成文，不混入 Source Brief、Source Catalog、Candidate Frame Brief 或内部评分表。交付报告会说明选中框、胜出理由、有实质差异的备选框，以及预留测试结果。
