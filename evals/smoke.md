...
19. A deep-read run builds a working-memory Article Closure Contract before Compose. The title promise is answered by the thesis and developed in at least two body locations; the contract never appears as a final-article section or report field.
...
21. The serialized article passes `scripts/check-article.ps1`; the delivery report records `Article Integrity: passed`, and `scripts/check-run.ps1` rejects a self-authored report when the actual article fails.
