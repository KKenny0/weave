...
| `survey visual lifecycle` | `使用 Survey 的 Deep Research 模式研究一个同时包含架构、反馈回路、尺度曲线和新旧指标对照的论文族。脊柱按推荐项继续。图不是标配，ASCII 严格按规则。` | Carry visual evidence and idea shape through Collect, Digest, Outline, Fill, and Refine; use Visual Pass only for final admission; require paired Org example blocks at no more than 80 ASCII columns; apply the standalone evidence-boundary test in Self-review. |
| `survey Anthropic course benchmark` | `使用 Survey 的 Canonical Article 模式，把 Anthropic Skilljar 的 Building with the Claude API 课程整理成一篇从零可用的中文权威参考。脊柱按推荐项继续。` | Use `anthropic-course-benchmark.md`; record accessible versus gated course evidence, score the baseline before changes, rerun after each concrete workflow repair, and stop only at the documented saturation gate. |
...
19. Survey visual judgment spans Collect evidence, Digest idea shape, Outline placement, prose-and-example-first Fill, Refine deletion, final Visual Pass admission, and Self-review evidence calibration. It admits only relationships that prose represents poorly, traces every visual element to evidence or labeled Weave synthesis, places exactly one standalone `<!-- weave-visual -->` marker before each retained visual, places evidence and boundary immediately after it, and accepts few or zero figures. Every ASCII visual uses one paired, non-nested Org `#+begin_example` / `#+end_example` block with no content line over 80 ASCII columns; Markdown-fenced, indented, or otherwise naked ASCII visuals fail. Impact Pass traces directly to evidence and the selected spine.
